...
if [ "$ARM" != "armv6l" ]; then
  read -p "you are running a Pi Zero, installation will continue, but you will have to run in server only mode." choice
	if [[ $choice =~ ^[Nn]$ ]]; then
...
if [[ $choice =~ ^[Yy]$ ]]; then
  ...
  if [ "USER"  != "pi" ]; then
    ...
    sed 's/pi/'$USER'/g' $PM2_FILE > ~/MagicMirror/PiZero/pm2_MagicMirrorPiZero.json
    PM2_FILE=~/MagicMirror/PiZero/pm2_MagicMirrorPiZero.json
  fi
	pm2 start $PM2_FILE
	pm2 save
fi
